...
def test_procedure(tst_img, tst_lab):
    ...
    for step in range(tst_batch_num):
        ...
        if plotROC:
            ...
            with open('../db_resnet_single_test_label.txt', 'a+') as f:
            ...
            with open('../db_resnet_single_y_score.txt', 'a+') as g:
...
with tf.variable_scope('Full_Connected_Layer'):
    sum_avg_flat = tf.reshape(max_pool_normal, [-1, int(max_pool_normal.get_shape()[3])])
