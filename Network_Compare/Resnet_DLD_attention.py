...
def attention_block(input, block_name):
    with tf.variable_scope(block_name):
        ...
        fc1 = fc_layer(input=global_pool, out_channel=in_channel // r, layer_name='fc1')
...
def inference(input,
    ...
    fc_bn = bn_layer(section3_attention, layer_name='fc_bn')
    fc_relu = relu_layer(fc_bn, layer_name='fc_relu')
    fc_pool = global_avg_pool_layer(fc_relu, layer_name='global_pool')
    fc_pool = flatten(fc_pool, layer_name='flatten')
    y_result = fc_layer(fc_pool, 7, layer_name='fc')

    return y_result, sec1_attention_input, sec1_sig_weight, sec3_attention_input
...
#                                                       is_training: False})
#
#         with open('../baseline_test_label.txt', 'a+') as f:
...
#                 f.write(tmp_str + '\n')
#
#         with open('../baseline_y_score.txt', 'a+') as g:
...
# def saveImages(imageSet, out_dir, step_num, sig_weight):
...
#                     tmp_img[i][j] = (tmp_img[i][j] - MIN_VALUE) / (MAX_VALUE - MIN_VALUE) * 255
...
#         original_image_save_dir = '../attentionImage/origin_image/'
#         saveImages(tst_x, original_image_save_dir, step, sig_weight=np.ones([128, 128], dtype=np.int32))
#
...
#         attentionInputDir = '../attentionImage/attention_input/'
#         attentionOutputDir = '../attentionImage/attention_output/'
#
#         saveImages(attentionInput, attentionInputDir, step, sig_weight=np.ones([128, 128], dtype=np.int32))
#         saveImages(attentionInput, attentionOutputDir, step, sig_weight=sig_weight)
...
# def generate_GradCAM_Image(save_dir='../Grad_CAM_Baseline_Split/'):
...
y_conv, sec1_att_input, sec1_sig_weight, convForCAM = inference(input=x,
                                                                ksize=ksize,
                                                                out_channel1=128,
                                                                out_channel2=256,
                                                                out_channel3=512)

# conv_cam, y_cam = pre_grad_cam(convForCAM, y_conv)
