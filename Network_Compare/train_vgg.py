...
restore_dir = '../RestoreLib/vgg_16.ckpt'
...
def plotROCInformation(tst_img, tst_lab):
    ...
    for step in range(tst_batch_num):
        ...
        with open('../vgg_test_label.txt', 'a+') as f:
        ...
        with open('../vgg_y_score.txt', 'a+') as g:
...
y_conv, end_point = vgg_16(inputs=x, num_classes=NUM_LABELS, is_training=is_training, global_pool=True)
...
if Training:
    variables_to_restore = slim.get_variables_to_restore(exclude=['vgg_16/fc6', 'vgg_16/fc7', 'vgg_16/fc8', 'vgg_16/conv1'])
    init_fn = slim.assign_from_checkpoint_fn(restore_dir, variables_to_restore, ignore_missing_vars=True)
    sess.run(tf.global_variables_initializer())
    init_fn(sess)
else:
    saver = tf.train.Saver()
    saver.restore(sess, '../Checkpoint/vgg-16/vgg-16100.ckpt')

print('vgg-16 model parameters restore finished')
