...
def spp_block(input, block_name):
    with tf.variable_scope(block_name):
        ...
        avgpool_4x4 = avg_pool_layer(input, ksize=pool4x4_kernel, stride=pool4x4_stride, layer_name='spp_pool4x4')
        avgpool_2x2 = avg_pool_layer(input, ksize=pool2x2_kernel, stride=pool2x2_stride, layer_name='spp_pool2x2')
        avgpool_1x1 = avg_pool_layer(input, ksize=pool1x1_kernel, stride=pool1x1_stride, layer_name='spp_pool1x1')

        flatten_4x4 = tf.layers.flatten(avgpool_4x4)
        flatten_2x2 = tf.layers.flatten(avgpool_2x2)
        flatten_1x1 = tf.layers.flatten(avgpool_1x1)
...
def attention_block(input, block_name):
    with tf.variable_scope(block_name):
        ...
        spp = spp_block(input=relu1, block_name='spp')
        fc1 = fc_layer(input=spp, out_channel=in_channel // 2, layer_name='fc1')
...
def partial_fc_layer(input, out_channel, layer_name):
    with tf.variable_scope(layer_name):
        ...
        partial_fc_pool = spp_block(partial_fc_relu, block_name='SPP')
...
def section(input, num, out_channel, ksize, section_name, down_sampling, first_conv):
    ...
    with tf.variable_scope(section_name):
        ...
        out = attention_block(out, block_name='attention_block')
...
def inference(input,
    ...
    attention_unit0 = attention_block(unit0, block_name='attention_unit0')

    section1 = section(input=attention_unit0,
                       out_channel=out_channel1,
                       num=num_block1,
                       ksize=ksize,
    ...
    partial_fc_sec1 = partial_fc_layer(section1, out_channel=NUM_LABELS, layer_name='partial_fc_section1')
    ...
    partial_fc_sec2 = partial_fc_layer(section2, out_channel=NUM_LABELS, layer_name='partial_fc_section2')
    ...
    partial_fc_sec3 = partial_fc_layer(section3, out_channel=NUM_LABELS, layer_name='partial_fc_section3')
