...
def test_procedure(tst_img, tst_lab):
    ...
    for step in range(tst_batch_num):
        ...
        if TestImgSummary:
            tst_merged = sess.run(merged, feed_dict={x: tst_x,
                                                     y_: tst_l,
                                                     is_training: False})
            writer_test.add_summary(tst_merged, step)
...
with tf.variable_scope('Summary_writer'):
    ...
    if Training:
        ...
        writer_test = tf.summary.FileWriter(checkpoint_test_dir)
