...
Grad_CAM_ImagePath = 'D:/Workspace/DLD_Classification/Grad_CAM_Baseline_Split/'

softmaxScoreFile = 'D:/Workspace/DLD_Classification/model_results_ROC/after-softmax/score/baseline_y_score.txt'
realLabelFile = 'D:/Workspace/DLD_Classification/model_results_ROC/after-softmax/label/baseline_test_label.txt'
savedPath = 'D:/Workspace/DLD_Classification/Grad_CAM_Baseline_Split_Comparison/'
...
def generateFusedImages(fileLists, rowsPerImage=10):
    ...
    for f in fileLists:
        ...
        cam_img_lib.append(Image.open(Grad_CAM_ImagePath + f))
        ...
        if img_counter == rowsPerImage:
            ...
            img_width += 3
            img_height += 3
            newImg = Image.new('RGB', (img_width * 24, img_height * rowsPerImage))
            draw = ImageDraw.Draw(newImg)
            for i in range(rowsPerImage):
                newImg.paste(origin_img_lib[i], box=(img_width * 2, 0 + img_height * i))
                newImg.paste(cam_img_lib[i], box=(img_width * 3, 0 + img_height * i))
                draw.text((0, 0 + img_height * i), file_name_lib[i])
                draw.text((img_width * 4, 0 + img_height * i), softmax_score_lib[i])
                draw.text((img_width * 20, 0 + img_height * i), real_label_lib[i])
...
if __name__ == '__main__':
    fList = getFileList(Grad_CAM_ImagePath)
    # print(fList)
